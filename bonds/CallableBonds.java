...
public class CallableBonds {
    ...
    public static YieldTermStructure flatRate(Date today, Quote forward, DayCounter dc, Compounding compounding,
            Frequency frequency) {
        return new FlatForward(today, new QuoteHandle(forward), dc, compounding, frequency);
    }

    public static void priceCallable(double sigma, YieldTermStructureHandle termStructure,
            CallableFixedRateBond callableBond, Frequency frequency, DayCounter bondDayCounter) {
        ...
        PricingEngine engine0 = new TreeCallableFixedRateBondEngine(hw0, gridIntervals);
        ...
        System.out.printf("QuantLib price/yld (%%)  %10.2f  /  %10.2f \n", callableBond.cleanPrice(),
                100.0 * callableBond.yield(bondDayCounter, Compounding.Compounded, frequency, accuracy, maxIterations));

        System.out.printf("QuantLib OAS from model clean price (bp)  %10.2f \n", 10000.0 * callableBond
                .OAS(callableBond.cleanPrice(), termStructure, bondDayCounter, Compounding.Compounded, frequency));

        double cp = callableBond.cleanPriceOAS(10 * 1e-4, termStructure, bondDayCounter, Compounding.Compounded,
                frequency);
        ...
        System.out.printf("QuantLib OAS from spreaded clean price (bp)  %10.2f \n",
                10000.0 * callableBond.OAS(cp, termStructure, bondDayCounter, Compounding.Compounded, frequency));

        System.out.printf("QuantLib effectiveDuration / convexity for 10bp OAS %f / %f \n",
                callableBond.effectiveDuration(10 * 1e-4, termStructure, bondDayCounter, Compounding.Compounded,
                        frequency),
                callableBond.effectiveConvexity(10 * 1e-4, termStructure, bondDayCounter, Compounding.Compounded,
                        frequency));
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        YieldTermStructureHandle termStructure = new YieldTermStructureHandle(
                flatRate(today, new SimpleQuote(bbIR.rate()), bbIR.dayCounter(), bbIR.compounding(), bbIR.frequency()));
        ...
        for (long i = 0; i < numberOfCallDates; ++i) {
            ...
            BondPrice myPrice = new BondPrice(callPrice, BondPrice.Type.Clean);
        ...
        /*
         * The 30/360 day counter Bloomberg uses for this bond cannot reproduce the US
         * Bond/ISMA (constant) cashflows used in PFC1. Therefore use ActAct(Bond)
         */
        ...
        Schedule sch = new Schedule(dated, maturity, new Period(frequency), bondCalendar, accrualConvention,
                accrualConvention, DateGeneration.Rule.Backward, false);

        DoubleVector couponsVector = new DoubleVector();
        couponsVector.add(coupon);

        CallableFixedRateBond callableBond = new CallableFixedRateBond(settlementDays, faceAmount, sch, couponsVector,
                bondDayCounter, paymentConvention, redemption, issue, callSchedule);

        priceCallable(1e-20, termStructure, callableBond, frequency, bondDayCounter);

        System.out.printf("Bloomberg price/yld (%%) 96.50 / 5.47 \n");

        priceCallable(0.01, termStructure, callableBond, frequency, bondDayCounter);

        System.out.printf("Bloomberg price/yld (%%) 95.68 / 5.66 \n");

        priceCallable(0.03, termStructure, callableBond, frequency, bondDayCounter);

        System.out.printf("Bloomberg price/yld (%%) 92.34 / 6.49 \n");

        priceCallable(0.06, termStructure, callableBond, frequency, bondDayCounter);

        System.out.printf("Bloomberg price/yld (%%) 87.16 / 7.83 \n");

        priceCallable(0.12, termStructure, callableBond, frequency, bondDayCounter);
